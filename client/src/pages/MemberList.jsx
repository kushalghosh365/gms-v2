...
import { Search, Phone, Calendar, CheckCircle, XCircle, LogIn, LogOut, DollarSign, X, Download, Monitor, User } from 'lucide-react';
        ...
        {filteredMembers.map(m => (
          <div key={m._id} className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 hover:shadow-xl transition-all group relative overflow-hidden flex flex-col">
            ...
            <div className="flex items-start space-x-4 mb-6">
              <img
                src={m.photo ? (m.photo.startsWith('http') ? m.photo : (m.photo.startsWith("http") ? m.photo : `/${m.photo.replace(/\\/g, "/")}`)) : 'https://via.placeholder.com/60'}
            ...
            <div className="mt-auto pt-4 border-t border-slate-100 grid grid-cols-3 gap-2">
              ...
              <button
                ...
                className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 active:scale-95 active:bg-blue-200 flex flex-col items-center justify-center transition-all duration-200 shadow-sm hover:shadow-md"
              >
                <DollarSign size={18} />
          ...
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl relative">
            ...
            <div className="flex justify-center mb-8">
              ...
              >
                ...
                >
                  <img
                    src={qrModal.member.photo ? (qrModal.member.photo.startsWith('http') ? qrModal.member.photo : (qrModal.member.photo.startsWith("http") ? qrModal.member.photo : `/${qrModal.member.photo.replace(/\\/g, "/")}`)) : 'https://via.placeholder.com/150'}
          ...
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in fade-in zoom-in duration-300">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[1, 3, 6, 12].map(m => (
                  <button
                    ...
                    className={`p-3 rounded-xl border-2 font-bold transition-all ${selectedMonths === m ? 'border-indigo-600 bg-indigo-50 text-indigo-600' : 'border-slate-100 hover:border-indigo-100'}`}
                  >
                    {m} Month{m > 1 ? 's' : ''}
                  </button>
                ))}
            ...
            <div className="space-y-2 mb-8">
              ...
              <input
                ...
                className="w-full p-4 rounded-xl border-2 border-slate-100 focus:border-indigo-600 outline-none font-bold text-xl"
              />
            </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col animate-in zoom-in duration-300">
            ...
            <div className="p-8 space-y-4">
              <div className="flex items-center gap-6 mb-6 pb-6 border-b border-slate-100">
                <img
                  src={detailsModal.member.photo ? (detailsModal.member.photo.startsWith('http') ? detailsModal.member.photo : (detailsModal.member.photo.startsWith("http") ? detailsModal.member.photo : `/${detailsModal.member.photo.replace(/\\/g, "/")}`)) : 'https://via.placeholder.com/100'}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  ...
                  <p className="font-bold text-slate-800">{detailsModal.member.phone}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">WhatsApp</p>
                  <p className="font-bold text-slate-800">{detailsModal.member.whatsapp || 'N/A'}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Email Address</p>
                  <p className="font-bold text-slate-800 truncate">{detailsModal.member.email || 'N/A'}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Category</p>
                  <p className="font-bold text-slate-800">{detailsModal.member.memberCategory || 'General'}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Package Type</p>
                  <p className="font-bold text-slate-800">{detailsModal.member.packageType || 'N/A'}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Expiry Date</p>
                  <p className="font-bold text-slate-800">{detailsModal.member.expiryDate ? new Date(detailsModal.member.expiryDate).toLocaleDateString('en-GB') : 'N/A'}</p>
                </div>
